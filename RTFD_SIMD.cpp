...
namespace SIMD_PARA { // SIMD and multithreaded implementation
	...
	*/
		...
		for (k = 0; k < 20; k++)
		{
			...
			for ( j=4; j<=N+4; j++ ) { 
				...
				for ( i=4; i<=N+4; i+=4 ) {
					...
					__m128 const left = SIMD::simd_lshift1(mid0, mid1);
					__m128 const right = SIMD::simd_rshift1(mid1, mid2);
	...
	{
		float a = dt * diff * N * N;
		SIMD_PARA::lin_solve(N, b, x, x0, a, 1 + 4 * a);
	}
	...
	void project(uint32_t N, float *u, float *v, float *p, float *div)
	{
		...
		SIMD_PARA::lin_solve(N, 0, p, div, 1, 4);
	...
	void dens_step(uint32_t N, float *x, float *x0, float *u, float *v, float diff, float dt)
	{
		SIMD::add_source(N, x, x0, dt);
		SWAP(x0, x);
		SIMD_PARA::diffuse(N, 0, x, x0, diff, dt);
		SWAP(x0, x);
		SIMD_PARA::advect(N, 0, x, x0, u, v, dt);
	}
